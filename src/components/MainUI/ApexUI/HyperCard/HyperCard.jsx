...
export default function LightningCard({
    ...
    return (
        <div
            className={`relative overflow-hidden rounded-md ${width} ${height} group cursor-pointer scale-100 z-30 flex flex-col justify-center items-center `}
        ...
        >
            ...
            <div ref={contentRef} className="relative z-20 flex flex-col items-center justify-center w-full h-full px-2 py-2 sm:px-4 sm:py-4">
                <h2 className="text-lg max-sm:text-md md:text-[1.3rem] font-normal z-20 flex flex-wrap items-center justify-center gap-1 text-center whitespace-nowrap">
                    <span>{text ? text : children}</span>
                    <span className={`font-bold text-${LastTextColor}-400 ml-1`}>{LastText}</span>
                </h2>
                <p className="text-xs sm:text-sm md:text-base text-gray-300 text-center max-w-xs sm:max-w-md z-20 mt-1">
                    {SubText}
                </p>
            </div>
        </div>
