...
const getAnchorAndDir = (w, h) => {
    ...
    return { anchor: [0.5 * w, -0.1 * h], dir: [0, 1] };
};
...
const LensFlareBackground = ({
    ...
    intensity = 1.0,        // Intensity of the flare              // Number of flare elements
    animationSpeed = 1.0,   // Animation speed // Background color (not used in this effect, but included as prop)             // Blur amount (not used in this effect, but included as prop)
}) => {
    const mountRef = useRef();
    const uniformsRef = useRef();
    const mouseRef = useRef({ x: 0.5, y: 0.5 });
    const smoothMouseRef = useRef({ x: 0.5, y: 0.5 });
    // Removed unused animationIdRef
    useEffect(() => {
        ...
        const setupScene = () => {
            ...
            const uniforms = {
                ...
                lightSpread: { value: 6 }, // default
                rayLength: { value: 0.9 },   // default
                pulsating: { value: 0.0 },   // default
                fadeDistance: { value: 1.0 },// default
                saturation: { value: 1.0 },  // default
                mousePos: { value: new THREE.Vector2(0.5, 0.5) },
                mouseInfluence: { value: 0.1 }, // default
                noiseAmount: { value: 0.0 },     // default
                distortion: { value: 0.0 },      // default
                intensity: { value: intensity }, // new prop  
            };
    ...
    `;
        ...
        // Always enable mouse move for effect
        window.addEventListener("mousemove", handleMouseMove);

        // Animation loop
        const animate = (now) => {
            uniforms.iTime.value = now * 0.001;
            // Always use mouse smoothing for effect
    ...
    return (
        <div
            ref={mountRef}
            className={`pointer-events-none w-[135vw]  overflow-hidden$`.trim()}
            style={{
                position: "absolute",
                bottom: 0,
                height: "100vh",
