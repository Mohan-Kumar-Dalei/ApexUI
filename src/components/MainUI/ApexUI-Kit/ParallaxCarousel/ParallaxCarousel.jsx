...
// --- Card Component with Hover Parallax Effect ---
const CarouselCard = ({ slide, isCenter }) => {
    const cardRef = useRef(null);
    const x = useMotionValue(0);
    const y = useMotionValue(0);

    // Mouse move ke hisab se rotation calculate karta hai (effect ko strong banaya gaya hai)
        ...
        >
            ...
            >
                <img
                    src={slide.imageUrl}
                    alt={slide.title}
                    className="w-full h-full object-cover group-hover:opacity-30 transition-all duration-300 ease-in-out rounded-2xl"
                />
                <img
                    src={slide.pngUrl}
                    alt={slide.title}
                    className="absolute inset-0 w-full h-full object-cover group-hover:translate-z-15 transition-all duration-300 ease-in-out rounded-2xl"
                />
            </div>
            <div
                className="relative flex flex-col justify-end h-full text-white p-8"
                style={{
                    transformStyle: 'preserve-3d',
                    perspective: '1500px',
                }}
...
const ParallaxCarousel = ({ slides }) => {
    ...
    return (
        ...
        >
            ...
            <motion.button
                onClick={handlePrev}
                className="absolute -left-10 lg:left-0 top-1/2 -translate-y-1/2 z-30 p-3 rounded-full text-slate-400 hover:text-lime-400  transition-colors"
                whileTap={{ scale: 0.9 }}
            >
                <ArrowLeft size={32} />
            </motion.button>
            <motion.button
                onClick={handleNext}
                className="absolute -right-10 lg:right-0 top-1/2 -translate-y-1/2 z-30 p-3 rounded-full text-slate-400 hover:text-lime-400 transition-colors"
                whileTap={{ scale: 0.9 }}
            >
                <ArrowRight size={32} />
            </motion.button>
